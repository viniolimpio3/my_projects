...
<script>
  ...
  //A biblioteca axios é basicamente uma API que sabe interagir tanto com XMLHttpRequest quanto com a interface http do node
  ...
  export default {
    ...
    data:function(){//método Que Retorna pra view p/ exemplo !!
      return{
    ...
    computed:{// essa propriedade,serve para carregar o método domain() apenas quando for necessário -> quando this.prefixes ou this.sufixes for alterado -> quando adiciona índices, ou retira do objeto.  
      ...
      //fazendo a requisição dos dados no backend
      console.log("Created..");
      axios({ //como se fosse o ajax
        url:"http://localhost:4000",
        method: "post",
        data: {//query é a consulta aos dados!
          query: `{
            prefixes {
              id
              type
